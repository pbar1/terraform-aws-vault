module "nametagger" {
  source = "git::git@github.com:pbar1/terraform-aws-nametagger?ref=v1.0.0"
...
locals {
  ssm_path_vault_recovery_keys_b64 = "${
    var.ssm_path_vault_recovery_keys_b64 != ""
    ? var.ssm_path_vault_recovery_keys_b64
    : "/${module.nametagger.name}/recovery_keys_b64"
  }"

  ssm_path_vault_root_token = "${
    var.ssm_path_vault_root_token != ""
    ? var.ssm_path_vault_root_token
    : "/${module.nametagger.name}/root_token"
  }"
}
...
resource "aws_kms_key" "vault" {
  tags        = "${module.nametagger.tags}"
  description = "${module.nametagger.name} key for Vault Auto Unseal"
}

resource "aws_kms_alias" "vault" {
  name          = "alias/${module.nametagger.name}"
...
resource "aws_dynamodb_table" "vault" {
  tags           = "${module.nametagger.tags}"
  name           = "${module.nametagger.name}-storage"
...
resource "aws_iam_policy" "vault" {
  name        = "${module.nametagger.name}"
  description = "DynamoDB, KMS, and SSM access for Vault cluster ${module.nametagger.name}"
...
resource "aws_iam_role" "vault" {
  tags               = "${module.nametagger.tags}"
  name               = "${module.nametagger.name}"
  description        = "Role that Vault cluster ${module.nametagger.name} runs as"
...
resource "aws_security_group" "vault_lb" {
  tags        = "${module.nametagger.tags}"
  name        = "${module.nametagger.name}-lb"
  description = "Vault LoadBalancer ${module.nametagger.name}"
  vpc_id      = "${var.vpc_id}"
...
resource "aws_security_group" "vault_cluster" {
  tags        = "${module.nametagger.tags}"
  name        = "${module.nametagger.name}-server"
  description = "Vault cluster ${module.nametagger.name}"
  vpc_id      = "${var.vpc_id}"
...
data "template_file" "userdata" {
  ...
  vars = {
    cluster_name             = "${module.nametagger.name}"
...
resource "aws_launch_template" "vault" {
  tags                   = "${module.nametagger.tags}"
  name                   = "${module.nametagger.name}"
  description            = "Vault cluster ${module.nametagger.name} launch template"
